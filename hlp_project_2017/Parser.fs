...
module Parser=
    ...
    let readAsm textInput =
        ...
        let executeWordsAsCommand (strlist:string list)= //: InstructionLine
            ...
            //LDM STM
            | [ IsMEMMInst inst; IsLDMdir dir; IsCondition cond; IsReg source ] ->  Line(MEM(inst(dir,source,[],false)),None,CondCast cond)
            | [ IsMEMMInst inst; IsLDMdir dir; IsCondition cond; IsReg source; "!" ] ->  Line(MEM(inst(dir,source,[],true)),None,CondCast cond)
            | IsMEMMInst inst::IsLDMdir dir::IsCondition cond::IsReg source::IsRegList reglist ->  Line(MEM(inst(dir,source,reglist,false)),None,CondCast cond)
            | IsMEMMInst inst::IsLDMdir dir::IsCondition cond::IsReg source::"!"::IsRegList reglist ->  Line(MEM(inst(dir,source,reglist,true)),None,CondCast cond)
            //LABEL Instructions
